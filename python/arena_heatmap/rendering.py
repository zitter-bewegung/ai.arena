...
def render_figure(preset, heatmaps, n_games, outdir, kind):
    """Save a 1×5 figure (one heatmap per unit type) to outdir/<kind>/<slug>.png."""
    _apply_paper_style()
    ...
    fig.suptitle(f"{preset.title} — {kind.title()}   (N = {n_games} games)", y=1.02)

    target_dir = os.path.join(outdir, kind)
    os.makedirs(target_dir, exist_ok=True)
    png_path = os.path.join(target_dir, f"{preset.slug}.png")
